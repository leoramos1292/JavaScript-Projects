...
//this function styles the game button while the are disabled
...
function stopEnabled(btn) {
    ...
    btn.style.bacgroundColr = "rgb(255,51,51)";
    btn.disabled = false;
}
...
// when the userinidicates, stop the current game and reset game
function stopGame() {
...
//this functino will write text to the game message console
...
//this functio will reutrn the active player's avatar
function determineAvatar() {
    //determine the correct avatar to paint for the active player
    var avatarArray = getAvatars(); //returns an array of oth player's assigned avatars
...
function avatarPlaced() {
    ...
    //check if there is laready a winner...if there is, then don't continue the game
    if (parseText == "That's three in a  row, PLayer 1 wins!" || parseText == "That's three in a row, Player 2 wins!"){
        showPlayer.innerHTML = "Game Stopped";
        showPlayer.style.color = 'red';
    }
    activePlayer = showPlayer.innerHTML; // get the cureent player from the element
...
//as squares are slecected they check in with this function to see if that particular
...
function recordMoves(square) {
    ...
    var boardState = document.getElementById('boardState').innerHTML // retireve boardState array
    var info = boardState.split(","); //seerate the tring by commas to create an array
    verdict = check(info,square); //call function to check if ptoposed square is already occupied
    return verdict;
}
...
//call this function to check state for any ties and act accordingly
function check4Tie() {
...
//call this function to produces the following winning provcess for the game
function winner(winDetected,winCon) {
    if(winDetected == "win") {
        ...
        glowBoard(winCon); //call functions to make the gameboard pulse with colors
    }
}

//this function will make the winning sqaures light up in celebration
function glowBoard(pos) {
    ...
    for (var i=0; i < squares.length; i++) {
        if (i == index0) {
            ...
            setTimeout(function() {bg3.style.backgroundColor = 'rgb(66,244,235)';}, 700)
...
function squareSound() {
    ...
    sound.play();
    setTimeout(function() {sound.pause();}, 400); //add delay to thse to keep sound short
...
function checkWinCon1(info,squareArray) {
    ...
    //do appear in the array, redord their
    ...
    //this will trigger (ONLY) if there was a match for index0, index1, index3
...
function checkWinCon5(info,squareArray) {
    ...
    winner(winDetected,winCon5); //winCon1 is the array of win combo
}
...
function checkWinCon6(info,squareArray) {
    ...
    winner(winDetected,winCon6); //winCon1 is the array of win combo
}
...
function checkWinCon7(info,squareArray) {
    ...
    winner(winDetected,winCon7); //winCon1 is the array of win combo
}
...
function checkWinCon8(info,squareArray) {
    ...
    winner(winDetected,winCon8); //winCon1 is the array of win combo
}
//-----------------------------------------------------------------------------------------
...
function square1Animate() {
    ...
    if (activePlayer != "Game Stopped") {//if game has not yet started prevent avatar placement
        ...
        //check if proposed square is valid
        var verdict = recordMoves(square);
        if (verdict == undefined) { // if verdict is empty than the square is unoccupied
            var paintAvatar = determineAvatar(); // get the correct avatar to paint fot the active player
            var selected = document.getElementsByClassName(paintAvatar)[0]; //paint avatar
            if (paintAvatar == "O") {//change these all to ternary statements
                animateO(selected); //call functino to animate X
            } else if (paintAvatar == "X") {
                animateX(selected); //call function to animate X
            }
            //build new array adding the newly selected aquare and the assigned avatar
...
function square6Animate() {
    ...
    if (activePlayer != "Game Stopped") {//if game has not yet started prevent avatar placement
        ...
        if (verdict == undefined) { // if verdict is empty than the square is unoccupied
            ...
            var selected = document.getElementsByClassName(paintAvatar)[0]; //paint avatar
            if (paintAvatar == "O") {//change these all to ternary statements
                animateO(selected); //call function to animate X
            } else if (paintAvatar == "X") {
                animateX(selected); //call function to animate X
            }
            //build new array adding the newly selected aquare and the assigned avatar
